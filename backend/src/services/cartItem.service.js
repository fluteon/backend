...
async function updateCartItem(userId, cartItemId, cartItemData) {
  const item = await findCartItemById(cartItemId);
  if (!item) throw new Error(`Cart item not found: ${cartItemId}`);
...
async function isCartItemExist(cart, product, size, userId) {
  return await CartItem.findOne({ cart, product, size, userId });
}
...
async function findCartItemById(cartItemId) {
  const cartItem = await CartItem.findById(cartItemId).populate("product");
  if (!cartItem) throw new Error(`CartItem not found with id: ${cartItemId}`);
  return cartItem;
}
