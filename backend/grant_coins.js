...
async function run() {
  ...
  // Update Tappu Sena
  const user = await User.findOne({ email: "aryesh.shivmare34@gmail.com" });
  if (user) {
    user.fonCoins = 995; // Give them exactly the 995 they expect
    await user.save();
    console.log("Updated Tappu fonCoins to 995");
  }
